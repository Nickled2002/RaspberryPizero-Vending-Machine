...
Version         : 0.1
Copyright       : :|
Description     :Driver for vending machine adapted from IL2
*/
...
static int device_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
	...
	case IOCTL_PIIO_GPIO_WRITE:
		...
		break;
	case IOCTL_PIIO_GPIO_SERVO_WRITE:
		copy_from_user(&apin, (gpio_pin *)arg, sizeof(gpio_pin));
		gpio_request(apin.pin, apin.desc);
		gpio_direction_output(apin.pin, 0);
		gpio_set_value(apin.pin, HIGH);
		printk("piio: IOCTL_PIIO_GPIO_SERVO_WRITE - pin:%u - val:%i - desc:%s - dir:%p\n" , apin.pin , apin.value , apin.desc , apin.dir);
		break;
	default:
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Nikos Ledakis Engonopoulos");
